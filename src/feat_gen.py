...
def gini(array: np.ndarray) -> float:
    """Compute the Gini coefficient (array must be non-negative, length >= 1)."""
    if len(array) == 0:
        return 0.0
...
def feature_generation(mint_address, tx_list, price_dict, sig2bundle, account2signer):
    ...
    if "mint" not in tx_list[0]["type"]:
        return None

    valid_tx_list = [tx for tx in tx_list if "swap" in tx["type"]]
    ...
    feat[f"{prefix}_migrate_month"] = migrate_ts.month
    feat[f"{prefix}_migrate_day"] = migrate_ts.day
    feat[f"{prefix}_migrate_hour"] = migrate_ts.hour
    feat[f"{prefix}_migrate_weekday"] = migrate_ts.weekday()
    ...
    for row in tx_list:
        ...
        if len(trader_map) > 1 and "swap" in row["type"]:
            cluster_set_list.append(set(trader_map.keys()))

        if row["signature"] in sig2bundle and trader_map:
    ...
    if tx_list[0]["type"] == "mint" and tx_list[1]["type"] == "swap":
    ...
    for v in bundle2cluster.values():
        if len(v) > 1:
            cluster_set_list.append(v)
    for v in signer2cluster.values():
        if len(v) > 1:
            cluster_set_list.append(v)
    ...
    def mean_pnl(df, addr_set):
        ...
        return sub.iloc[:, 1].mean() if len(sub) > 0 else 0.0
    ...
    feat[f"{prefix}_transfer_tx_num"] = transfer_tx_cnt
    ...
    # ======== Feature Group 4: Bundle / Cluster Statistics ========
    prefix = "group4"
    uf = UnionFind()
    n = len(cluster_set_list)
    for i in range(n):
        for j in range(i + 1, n):
            if cluster_set_list[i] & cluster_set_list[j]:
                uf.union(i, j)
    root_to_set = defaultdict(set)
    for i, s in enumerate(cluster_set_list):
        root_to_set[uf.find(i)].update(s)
    ...
    hold_df["cluster"] = hold_df["trader"].map(addr2cluster).fillna(hold_df["trader"])
    real_hold_df = (hold_df.groupby("cluster", as_index=False)["amount"]
                    .sum().sort_values("amount", ascending=False))
    cluster_hold_df = real_hold_df[real_hold_df["cluster"].str.startswith("cluster")]

    trader_df["cluster"] = trader_df["trader"].map(addr2cluster).fillna(trader_df["trader"])
    ...
    feat[f"{prefix}_cluster_holder_ratio"] = feat[f"{prefix}_cluster_holder_num"] / len(hold_df)
    ...
    feat[f"{prefix}_top1_pct_delta"] = feat[f"{prefix}_top1_pct"] - feat["group2_top1_pct"]
    feat[f"{prefix}_top5_pct_delta"] = feat[f"{prefix}_top5_pct"] - feat["group2_top5_pct"]
    feat[f"{prefix}_top10_pct_delta"] = feat[f"{prefix}_top10_pct"] - feat["group2_top10_pct"]
    feat[f"{prefix}_top20_pct_delta"] = feat[f"{prefix}_top20_pct"] - feat["group2_top20_pct"]
    feat[f"{prefix}_top50_pct_delta"] = feat[f"{prefix}_top50_pct"] - feat["group2_top50_pct"]
    feat[f"{prefix}_top100_pct_delta"] = feat[f"{prefix}_top100_pct"] - feat["group2_top100_pct"]

    # ======== Time Series Features ========
    try:
        df = pd.DataFrame(valid_tx_list)
        df['datetime'] = pd.to_datetime(df['timestamp'], unit='s')
        df['volume'] = df['token_amount'].abs()
        ...
        if pd.isna(agg['close'].iloc[0]):
            agg['close'].iloc[0] = df['price'].iloc[0]
        ...
        ts = agg[feat_cols].to_numpy(dtype='float32')
        L, D = ts.shape
        if L >= MAX_T:
            ts_fixed = ts[-MAX_T:]
            seq_len = MAX_T
        else:
            pad = np.zeros((MAX_T - L, D), dtype='float32')
            if L > 0:
                pad[:, 0:4] = ts[-1, 0:4]
        ...
        print(f"[WARN] time series failed for {mint_address}")
...
if __name__ == '__main__':
    price_dict = {}
    with open(os.path.join(PROJECT_ROOT, "data/feat", "sol_hourly.txt")) as f:
        for line in f:
            ts_str, price_str = line.rstrip().split("\t")
            price_dict[int(ts_str)] = float(price_str)

    with open(os.path.join(PROJECT_ROOT, "data/bundle", "bundle2sig_filtered.pkl"), "rb") as f:
    ...
    exchange_signer = set()
    with open(os.path.join(PROJECT_ROOT, "data/bundle", "exchange_account.txt")) as f:
        for line in f:
            exchange_signer.add(line.strip())

    account2signer = {}
    with open(os.path.join(PROJECT_ROOT, "data/bundle", "first_sig_signer.jsonl")) as f:
        for line in f:
            info = json.loads(line)
            account, signer = info["account"], info["signer"]
            if account and signer and signer not in exchange_signer:
                account2signer[account] = signer

    tx_dir = os.path.join(PROJECT_ROOT, "data/tx", "pre_migration_tx")
    tx_file_list = os.listdir(tx_dir)

    feat_list = []
    for token_tx_file in tqdm(tx_file_list):
        token_address = token_tx_file.split(".")[0]
        tx_list = []
        with open(os.path.join(tx_dir, token_tx_file)) as f:
            for line in f:
    ...
    with open(os.path.join(PROJECT_ROOT, "data", "feat_with_ts.pkl"), "wb") as f:
